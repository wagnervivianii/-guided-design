...
function drawTable() {
  ...
  for(person in listOfPeople) {
      ...
      <tr class="dinamic-content" style="background-color: ${(person % 2 == 0) ? '#000': 'orange'}; color: white">
        <td>${listOfPeople[person].name}</td>
        <td>${listOfPeople[person].tel}</td>
        <td style="color: ${(listOfPeople[person].xp == true) ? "green" : "red" }; font-weight: bold; " >${listOfPeople[person].xp}<td>
