...
function registerData(e){
 ...
 let numregex = /[^0-9]/g;
  ...
  if(e.target.elements[1].value.match(numregex) != -1 && document.querySelector('.input_telefone').value.length != 15){
  ...
  if(e.target.elements[0].value.match(strregex) != -1 && e.target.elements[0].value.length < 3){
...
// função que não permite que teclas ñ alpha sejam digitadas
function mascText(e){
  // tirando conportamento padrão do input
  e.preventDefault();
  // criando pesquisa regex
  let regexNum = /[^a-z]/gi;
  // usando a função test paara testar o regex
    if(!regexNum.test(e.key)){
  // Permitindo que apenas teclas que passaram no test sejam somadas 
...
function mascNum(e){
  ...
  // usando a função test paara testar o regex e limitando a 15 digitos no input
  if(!regexNum.test(e.key) && e.target.value.length < 15){
  // Permitindo que apenas teclas que passaram no test sejam somadas 
      e.target.value += (e.key)
  }
}
